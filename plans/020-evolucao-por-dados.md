...
- `lib/script-matches.ts`: `MATCH_AUTO = 0.8`, `MATCH_PENDENTE = 0.4`; `decidirCasamentos(rows)` **pura** (1 vídeo → 1 roteiro pelo maior score; `principal` = Instagram se houver, senão maior score); `casarRoteiros()` — RPC → upsert (auto: `confirmado=true, metodo='tsrank_auto'`; cinza: `null`) → para auto e **só se `published_url is null`**: `update vm_generated_scripts set status='published', published_url=link_video, published_at=data_publicacao`. Best-effort no padrão de `varrerEdicoes` (`lib/etl.ts:290`). `confirmarCasamento(scriptId, videoId, aceito)`.
...
Verificação: `backfill-matches --dry-run` lista ~21–30 auto + ~26 pendentes; sem dry-run `vm_script_performance` ≥20 linhas; `status='published'` ≥20; pendência aparece no Kasparov.
