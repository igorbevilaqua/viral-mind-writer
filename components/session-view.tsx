...
import { finalizeSession, markPublished, quickFeedback, swapHook, updateScript, suggestFragment, reportarProblema } from "@/lib/actions";
...
// "Reportar problema": grava um bug ligado à sessão (hub.bugs) via server action autenticada.
function ReportProblem({ sessionId }: { sessionId: string }) {
  const dialogRef = useRef<HTMLDialogElement>(null);
  ...
  return (
    <div className="max-w-[860px] mx-auto w-full px-4 sm:px-6 py-8 sm:py-10 space-y-5">
      ...
      <div className="pt-2 border-t border-white/[.06] flex justify-end">
        <ReportProblem sessionId={session.id} />
      </div>
    </div>
  );
}
